...
function calculateTrustScore(
  ...
  const coverageScore = Math.min(
    prov.results.coverage_branch / tierConfig.min_branch,
    1
  );
  breakdown.coverage = coverageScore;
  if (coverageScore < 0.8) {
    recommendations.push(
      `Increase branch coverage to ${
        tierConfig.min_branch * 100
      }% (currently ${(prov.results.coverage_branch * 100).toFixed(1)}%)`
    );
  }

  // Mutation score (0-1, normalized to tier target)
  const mutationScore = Math.min(
    prov.results.mutation_score / tierConfig.min_mutation,
    1
  );
  breakdown.mutation = mutationScore;
  if (mutationScore < 0.8) {
    recommendations.push(
      `Improve mutation score to ${
        tierConfig.min_mutation * 100
      }% (currently ${(prov.results.mutation_score * 100).toFixed(1)}%)`
    );
  }
  ...
  if (tierConfig.requires_contracts && contractScore === 0) {
    recommendations.push(
      "Implement and pass contract tests for all API boundaries"
    );
  }
  ...
  if (a11yScore < 1) {
    recommendations.push(
      "Implement accessibility testing (axe-core integration)"
    );
  }

  // Performance score (0 or 1) - placeholder
  const perfScore =
    prov.results.perf && Object.keys(prov.results.perf).length > 0 ? 1 : 0.5;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!command || command === "calculate") {
    try {
      ...
      console.log("📊 METRICS:");
      console.log(
        `  Branch Coverage: ${(prov.results.coverage_branch * 100).toFixed(1)}%`
      );
      console.log(
        `  Mutation Score: ${(prov.results.mutation_score * 100).toFixed(1)}%`
      );
