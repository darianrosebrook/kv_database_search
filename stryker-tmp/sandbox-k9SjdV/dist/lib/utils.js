 ...
 */
export function generateBreadcrumbs(filePath, vaultPath) {
 ...
 */
export function determineContentType(filePath, vaultPath, frontmatter) {
    const relativePath = filePath.replace(vaultPath, "").replace(/^\/+/, "");
    if (relativePath.startsWith("MOCs/"))
        return "moc";
    if (relativePath.startsWith("Articles/"))
        return "article";
    if (relativePath.startsWith("AIChats/"))
        return "conversation";
