...
describe("Accessibility (Axe-core) E2E Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const testAppPath = path.join(
      __dirname,
      "../../apps/rag_web_search_tool/index.html"
    );
  ...
  describe("Search Interface Accessibility", () => {
    it("should pass axe-core accessibility audit for search page", async () => {
      ...
      if (moderateViolations.length > 0) {
        console.warn(
          `Found ${moderateViolations.length} moderate accessibility violations`
        );
      }
  ...
  describe("Search Results Accessibility", () => {
    beforeEach(async () => {
      ...
      if (searchInput) {
        ...
        await page.waitForSelector(
          '[data-testid="search-result"], .result-item, [role="listitem"]',
          {
            timeout: 5000,
          }
        );
    ...
    it("should have accessible search results", async () => {
      const axe = new AxeBuilder({ page }).include(
        '[data-testid="search-results"], .results-container, [role="list"]'
      );
    ...
    it("should support keyboard navigation through results", async () => {
      ...
      expect(
        ["div", "article", "button", "a", "li"].includes(activeElement || "")
      ).toBe(true);
    });
  });
    ...
    it("should have accessible error messages", async () => {
      ...
      if (searchInput) {
        ...
        const errorMessages = await page.$$(
          '[role="alert"], .error, .error-message, [aria-live]'
        );
