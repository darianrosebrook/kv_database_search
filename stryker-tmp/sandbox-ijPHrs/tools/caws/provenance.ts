  ...
  try {
    ...
    if (fs.existsSync(coveragePath)) {
      ...
      if (fs.existsSync(coverageFinalPath)) {
        const finalCoverage = JSON.parse(fs.readFileSync(coverageFinalPath, "utf-8"));
        // Calculate totals from individual file coverage
        let totalBranches = 0, coveredBranches = 0, totalLines = 0, coveredLines = 0;
        for (const fileData of Object.values(finalCoverage) as any[]) {
          ...
          if (fileData.s) {
            for (const statementCount of Object.values(fileData.s) as number[]) {
              totalLines++;
    ...
    if (coverageData) {
      ...
      results.tests_passed = Math.floor((coverageData.total.lines.pct / 100) * 10); // Estimate test count
    }
  } catch (e) {
