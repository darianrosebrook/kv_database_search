...
import { ObsidianSearchService } from "../lib/obsidian-search.js";
...
`);
    ...
    try {
        ...
        console.log(`Results: ${searchResponse.results.length}/${searchResponse.totalFound} found`);
        if (searchResponse.results.length === 0) {
            ...
            if (searchResponse.facets) {
                ...
                if (searchResponse.facets.fileTypes &&
                ...
                if (searchResponse.facets.folders &&
            ...
            if (searchResponse.graphInsights) {
                ...
                if (searchResponse.graphInsights.queryConcepts.length > 0) {
                    console.log(`  Query concepts: ${searchResponse.graphInsights.queryConcepts.join(", ")}`);
                }
                if (searchResponse.graphInsights.relatedConcepts.length > 0) {
                    console.log(`  Related concepts: ${searchResponse.graphInsights.relatedConcepts
                        .slice(0, 5)
                        .join(", ")}`);
                }
        ...
        console.log("  Articles: tsx src/scripts/search.ts 'article design system'");
