...
async function collectTestResults(): Promise<ProvenanceManifest["results"]> {
  ...
  const mutationPath = path.join(__dirname, "../../reports/mutation/mutation.json");
  if (!fs.existsSync(mutationPath)) {
    ...
    try {
      ...
      console.warn("Mutation testing failed, proceeding without mutation score");
    }
  }
  ...
  const contractResultsPath = path.join(__dirname, "../../test-results/contract-results.json");
  ...
  try {
    if (fs.existsSync(contractResultsPath)) {
      const contractResults = JSON.parse(fs.readFileSync(contractResultsPath, "utf-8"));
      results.contracts = {
