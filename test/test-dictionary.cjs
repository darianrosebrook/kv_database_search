const { ObsidianDatabase } = require('./dist/apps/kv_database/src/lib/database');
const fs = require('fs');
const path = require('path');

// Try to load the dictionary service source directly
try {
  const dictionaryServicePath = './apps/kv_database/src/lib/dictionary-service.ts';
  ...
  const dictionaryServiceContent = fs.readFileSync(dictionaryServicePath, 'utf8');
  console.log('✅ Dictionary service file is readable');
  console.log('📊 Dictionary service file size:', dictionaryServiceContent.length, 'characters');
