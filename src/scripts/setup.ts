...
async function main() {
  ...
  try {
    ...
    if (embeddingTest.success) {
      ...
      console.log(
        `2. Model '${EMBEDDING_MODEL}' is available (ollama pull ${EMBEDDING_MODEL})`
      );
      process.exit(1);
    }
    ...
    if (stats.totalChunks === 0) {
      ...
      console.log(
        "  2. Test search functionality: npm run search 'design systems'"
      );
      console.log("  3. Start the web interface: npm run dev");
    } else {
      console.log(
        "  1. Test search functionality: npm run search 'design systems'"
      );
