...
class TrustScoreCalculator {
  ...
  private loadTierPolicies() {
    const policyPath = path.join(process.cwd(), ".caws", "policy", "tier-policy.json");
  ...
  calculateTrustScore(
    ...
    const breakdown = {
      ...
      contracts: policy.requires_contracts
        ? (components.contracts_consumer && components.contracts_provider ? 1 : 0)
        : 1,
  ...
  private generateRecommendations(
    ...
    if (breakdown.coverage < 0.8) {
      recommendations.push(
        `Increase branch coverage to ${policy.min_branch}% (currently ${(components.coverage_branch * 100).toFixed(1)}%)`
      );
    }
    ...
    if (breakdown.mutation < 0.8) {
      recommendations.push(
        `Improve mutation score to ${policy.min_mutation}% (currently ${(components.mutation_score * 100).toFixed(1)}%)`
      );
    }
    ...
    if (!components.a11y_passed) {
      recommendations.push("Fix accessibility violations and implement axe-core tests");
    }

    // Performance recommendations
    if (!components.perf_within_budget) {
      recommendations.push("Optimize performance to meet API budget requirements");
    }

    // Flake recommendations
    if (components.flake_rate > 0.05) {
      recommendations.push(`Reduce test flake rate below 5% (currently ${(components.flake_rate * 100).toFixed(1)}%)`);
    }

    // Overall score recommendations
    if (totalScore < 80) {
      recommendations.push(`Address critical issues to achieve Tier ${policy.tier} compliance (80+ score required)`);
    }
...
async function main() {
  ...
  try {
    ...
    console.log(`  • Coverage: ${(result.breakdown.coverage * 100).toFixed(1)}%`);
    console.log(`  • Mutation: ${(result.breakdown.mutation * 100).toFixed(1)}%`);
    console.log(`  • Contracts: ${result.breakdown.contracts === 1 ? "✅" : "❌"}`);
    console.log(`  • A11y: ${result.breakdown.a11y === 1 ? "✅" : "❌"}`);
    console.log(`  • Performance: ${result.breakdown.perf === 1 ? "✅" : "❌"}`);
    console.log(`  • Flake Rate: ${(result.breakdown.flake * 100).toFixed(1)}%`);
    console.log();
