...
async function main() {
  ...
  try {
    ...
    const obsidianPipeline = new ObsidianIngestionPipeline(
      database,
      embeddingService,
      OBSIDIAN_VAULT_PATH
    );
    ...
    const ingestionOptions = {
      batchSize: 1, // Process one file at a time for debugging
    ...
    if (validation.sampleResults.length > 0) {
      ...
      validation.sampleResults.slice(0, 3).forEach((sample, i) => {
        ...
        if (sample.obsidianMetadata) {
          ...
          console.log(
            `     Tags: ${
              sample.obsidianMetadata.tags?.slice(0, 3).join(", ") || "none"
            }`
          );
        }
        console.log("");
    ...
    for (const query of testQueries) {
      try {
        ...
        if (searchResults.length > 0) {
          ...
          console.log(
            `    Top: ${topResult.meta.section} (${(
              topResult.cosineSimilarity * 100
            ).toFixed(1)}%)`
          );
          console.log(
            `    File: ${topResult.meta.obsidianFile?.fileName || "unknown"}`
          );
          console.log(`    Type: ${topResult.meta.contentType}`);
          if (topResult.meta.obsidianFile?.tags?.length) {
            console.log(
              `    Tags: ${topResult.meta.obsidianFile.tags
                .slice(0, 3)
                .join(", ")}`
            );
          }
