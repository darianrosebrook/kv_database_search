...
function testFlakeDetection() {
  ...
  try {
    const result = execSync(
      "npx tsx apps/tools/caws/flake-detector.ts status",
      {
        encoding: "utf-8",
        cwd: process.cwd(),
      }
...
function testFeatureFlags() {
  ...
  try {
    const result = execSync("npx tsx apps/tools/caws/feature-flags.ts stats", {
      encoding: "utf-8",
      cwd: process.cwd(),
    });
...
function testCIPipeline() {
  ...
  const requiredFiles = [
    ...
    "apps/tools/caws/feature-flags.ts",
  ];
...
function testCAWSIntegration() {
  ...
  try {
    const result = execSync(
      "npx tsx apps/tools/caws/validate.ts working-spec .caws/working-spec.yaml",
      {
        encoding: "utf-8",
        cwd: process.cwd(),
      }
