...
const mockSources: Source[] = [
  {
    ...
    excerpt: "Comprehensive guide covering Pinecone, Weaviate, and Chroma implementations...",
    confidence: 0.94,
    type: "document",
  },
  {
    ...
    excerpt: "In-depth conversation about retrieval patterns and context optimization...",
    confidence: 0.87,
    type: "chat",
  },
]
...
const quickActions = ["Summarize this document", "Find related concepts", "Explain in simple terms", "Generate outline"]
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I'm your AI assistant. I can help you explore your knowledge base, answer questions about your documents, and provide insights based on your content. What would you like to know?",
      timestamp: new Date(),
    },
  ])
    ...
    <div className={cn("h-full flex flex-col bg-background", className)}>
        ...
        {messages.map((message) => (
          <div key={message.id} className={cn("flex gap-3", message.role === "user" && "flex-row-reverse")}>
            ...
            <div className={cn("flex-1 space-y-2", message.role === "user" && "flex flex-col items-end")}>
              ...
              >
                <Body className={cn("whitespace-pre-wrap", message.role === "user" && "text-primary-foreground")}>
                  {message.content}
                </Body>
              </div>
                  ...
                  <div className="grid gap-2">
                    {message.sources.map((source) => (
                      ...
                      >
                        ...
                        <Body className="text-sm text-muted-foreground line-clamp-2">{source.excerpt}</Body>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <Caption className={cn("text-xs", message.role === "user" && "text-right mr-12")}>
      ...
      <div className="p-4 border-t border-border">
        ...
        {attachments.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-2">
            {attachments.map((file, index) => (
              <div key={index} className="flex items-center gap-2 px-3 py-1 bg-accent rounded-md">
                <span className="text-sm truncate max-w-32">{file.name}</span>
                <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => removeAttachment(index)}>
                  ×
                </Button>
              </div>
            ))}
        ...
        <div className="flex items-end gap-2">
          ...
          <Button onClick={handleSend} disabled={!input.trim() && attachments.length === 0} className="h-11 px-4">
