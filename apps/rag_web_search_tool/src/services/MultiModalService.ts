...
export class MultiModalService {
  ...
  private detectContentTypeFromBytes(
    bytes: Uint8Array,
    fileName: string
  ): ContentTypeInfo {
    ...
    if (
      ...
      if (extension === "xlsx")
        return this.getContentTypeInfo("office_spreadsheet");
      if (extension === "pptx")
        return this.getContentTypeInfo("office_presentation");
  ...
  > {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Processor info request failed: ${response.statusText}`
        );
      }
