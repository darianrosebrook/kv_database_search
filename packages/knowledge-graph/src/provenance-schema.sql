-- Provenance and Explanation Database Schema
-- Provides comprehensive audit trails and explainable AI capabilities
...
-- Table for audit events (separate from provenance for detailed auditing)
...
-- View for provenance summary statistics
...
-- View for explanation effectiveness metrics
...
CREATE OR REPLACE VIEW lineage_paths AS
WITH RECURSIVE lineage_tree AS (
    -- Base case: start with direct lineage relationships
    SELECT 
        source_id,
        source_type,
        target_id,
    ...
    SELECT 
        ...
        dlg.target_id,
        dlg.target_type,
        dlg.relationship_type,
        dlg.transformation_method,
        dlg.confidence * lt.confidence as confidence, -- Compound confidence
        lt.depth + 1,
        lt.path || dlg.source_id,
        lt.root_source
    FROM data_lineage_graph dlg
    JOIN lineage_tree lt ON dlg.source_id = lt.target_id
    WHERE lt.depth < 10 -- Prevent infinite recursion
    AND NOT dlg.source_id = ANY(lt.path) -- Prevent cycles
)
SELECT * FROM lineage_tree;

-- Function to clean up old provenance records
...
-- Function to calculate provenance statistics
